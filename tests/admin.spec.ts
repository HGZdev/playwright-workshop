...
test.describe('Admin Flow', () => {
  test('should login as admin and see dashboard', async ({ page }) => {
    ...
    await page.getByTestId('login-input').fill('admin@example.com');
  ...
  test('should edit user', async ({ page }) => {
    ...
    const userRow = page.getByRole('row', { name: 'client1@example.com' });
    await userRow.getByRole('button', { name: 'Edit' }).click();

    await expect(page).toHaveURL(/\/admin\/user\//);

    // Wait for form to be populated
    await expect(page.getByLabel('Email')).toHaveValue('client1@example.com');
  ...
  test('should delete user', async ({ page }) => {
    ...
    const email = `todelete_${Date.now()}@example.com`;
